...
describe('last admin guard', () => {
  describe('revoke last admin blocked until second admin exists', () => {
  ...
  describe('two admins then revoke one', () => {
    /** @type {import('supertest').TestAgent} */
    let agent
    let adminId
    let companyId
    ...
    beforeAll(async () => {
      agent = createAgent(app)
      fixtureSuffix = `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
      const { email: a1email } = await registerCompanyAndAdmin(agent, {
        email: `solo_${fixtureSuffix}@example.com`,
        companyName: `Vitest LastAdmin ${fixtureSuffix}`
      })
      const u1 = await prisma.users.findFirst({ where: { email: a1email } })
      adminId = u1.id
      companyId = u1.company_id
      primaryDepartmentId = u1.department_id
    })
    ...
    it('allows two admins then revoke one; remaining user stays admin', async () => {
      const u1 = await prisma.users.findUnique({ where: { id: adminId } })

      const a2email = `admin_b_${fixtureSuffix}@example.com`
