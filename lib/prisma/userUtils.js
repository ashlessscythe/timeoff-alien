...
/** Statuses that consume allowance (matches `promise_my_active_leaves` in sessionUser). */
function allowanceCountingStatuses(explicit) {
...
function calculateNumberOfDaysTakenFromAllowance(
  ...
  function deductedForLeaveDay(leave, dayStr, isFirst, isLast) {
    ...
    // Day-part handling
    // Prisma may store day_part_* as null; treat missing/invalid as "all day".
    const sRaw = leave.day_part_start
    const eRaw = leave.day_part_end
    const sParsed = sRaw === null || sRaw === undefined ? dayPartAll : Number(sRaw)
    const eParsed = eRaw === null || eRaw === undefined ? dayPartAll : Number(eRaw)
    const s = sParsed === 2 || sParsed === 3 || sParsed === dayPartAll ? sParsed : dayPartAll
    const e = eParsed === 2 || eParsed === 3 || eParsed === dayPartAll ? eParsed : dayPartAll
    if (isFirst && isLast) {
  ...
  // Filter leaves for specified year and statuses that count toward allowance
  const statuses = allowanceCountingStatuses(opts.statuses)
...
async function validateLeaveBalance(
  ...
  try {
    ...
    const leaves = await prisma.leaves.findMany({
      where: {
        user_id: user.id,
        status: { in: allowanceCountingStatuses() },
        OR: [
          {
            date_start: {
              gte: yearStart,
...
async function getUserAllowanceWithContext(user, year = moment().year(), ctx = {}) {
  try {
    ...
    const { adjustment: fallbackAdjustment, carried_over_allowance: fallbackCarry } =
      ctx.adjustment_and_carry_over !== undefined
        ? ctx.adjustment_and_carry_over
        : await getAdjustmentAndCarryOverWithFallback()
    ...
    const leaves =
      ...
      (await prisma.leaves.findMany({
        where: {
          user_id: user.id,
          status: { in: allowanceCountingStatuses(ctx.statuses) },
          AND: [
...
async function getSchedulesForUsers(users, companyId) {
  ...
  try {
    ...
    const companyDefault =
      schedules.find(schedule => schedule.user_id === null) || DEFAULT_SCHEDULE
    ...
    return userIds.reduce((acc, userId) => {
      ...
      acc[userId] = userSchedule || companyDefault
      return acc
    }, {})
  } catch (error) {
    console.error('Error getting schedules for users:', error)
    return userIds.reduce((acc, userId) => {
      acc[userId] = DEFAULT_SCHEDULE
      return acc
    }, {})
  }
}

async function getAllowanceAdjustmentsForUsers(userIds) {
  if (!userIds.length) return {}

  const rows = await prisma.user_allowance_adjustment.findMany({
    where: { user_id: { in: userIds } },
    orderBy: [{ user_id: 'asc' }, { year: 'desc' }]
  })
...
function resolveAdjustmentForYear(user, year, adjustmentsByUser) {
  const y = Number(year)
  if (!Number.isFinite(y)) {
    return { adjustment: 0, carried_over_allowance: 0, record: null }
  }

  const userAdjustments = adjustmentsByUser[user.id] || []
  const exact = userAdjustments.find(record => record.year === y)
  if (exact) {
    return {
      adjustment: exact.adjustment || 0,
      carried_over_allowance: exact.carried_over_allowance || 0,
      record: exact
    }
  }

  const startYear = user.start_date ? moment.utc(user.start_date).year() : y
  for (let yr = y - 1; yr >= startYear; yr--) {
    const record = userAdjustments.find(adj => adj.year === yr)
    if (record) {
      return {
        adjustment: record.adjustment || 0,
        carried_over_allowance: record.carried_over_allowance || 0,
        record: null
      }
    }
  }
...
module.exports = {
  fullName,
  isActive,
  allowanceCountingStatuses,
  calculateNumberOfDaysTakenFromAllowance,
  getUserAllowance,
  getUserAllowanceWithContext,
  getUserSchedule,
  getSchedulesForUsers,
  getAllowanceAdjustmentsForUsers,
  resolveAdjustmentForYear,
