 ...
 */
function calculateNumberOfDaysTakenFromAllowance(user, leaves) {
  const currentYear = moment().year()
  const yearStart = moment.utc().startOf('year').format('YYYY-MM-DD')
  const yearEnd = moment.utc().endOf('year').format('YYYY-MM-DD 23:59:59')

  // Filter leaves for current year and approved status
  const relevantLeaves = leaves.filter(leave => {
    ...
    const isApproved = [1, 2, 3].includes(leave.status) // approved, new, pended_revoke
    return isInYear && isApproved
  })
...
async function getUserAllowance(user, year = moment().year()) {
  try {
    ...
    const numberOfDaysTakenFromAllowance = calculateNumberOfDaysTakenFromAllowance(user, leaves)
    ...
    return {
      number_of_days_available_in_allowance: Math.max(0, number_of_days_available_in_allowance),
