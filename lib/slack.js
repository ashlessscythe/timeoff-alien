...
Slack.prototype.promise_rendered_slack_template = function(args) {
  ...
  return (
    bluebird
      ...
      .then(() =>
        handlebars.render(
          path.join(__dirname, '..', 'views', 'slack', filename + '.hbs'),
          context
        )
      )
...
Slack.prototype.get_send_slack = function() {
  ...
  if (!slack_token) {
    return function() {
      console.debug('Pretend to send slack: ' + JSON.stringify(arguments))
      return bluebird.resolve()
    }
  }

  const slack_bot_name = config.get('slack:bot_name') || ''
  const slack_icon_url = config.get('slack:icon_url') || ''

  const web = new WebClient(slack_token)

  function push_slack(args) {
    if (!args.channel) {
      console.debug('No username set, skip sending to Slack.')
      return Promise.resolve()
    }

    console.debug('Sending slack message on channel "' + args.channel + '"')
    ...
    // Optional fields for classic apps / webhooks; ignored by newer bot tokens
    if (slack_bot_name) {
      payload.username = slack_bot_name
    }
    if (slack_icon_url) {
...
Slack.prototype.promise_leave_request_slacks = function(args) {
  ...
  return bluebird.join(
    ...
    leave
      ...
      .promise_supervisors()
      .then(supervisors => {
        // Ensure supervisors is an array before mapping
        if (!supervisors || !Array.isArray(supervisors)) {
          console.log('Warning: supervisors is not an array:', supervisors)
          return bluebird.resolve([])
        }
        return bluebird.all(
          supervisors.map(supervisor => promise_slack_to_supervisor(supervisor))
        )
      }),
    () => bluebird.resolve()
  )
...
Slack.prototype.promise_forgot_password_slack = function(args) {
  ...
  return user
    .getCompany()
    .then(company =>
      self.promise_rendered_slack_template({
...
Slack.prototype.promise_leave_request_edit_slacks = function(args) {
  const self = this
  const leave = args.leave
  const send_slack = self.get_send_slack()
  ...
  const promise_slack_to_supervisor = supervisor =>
    self
      .promise_rendered_slack_template({
        template_name: 'leave_request_edit_to_supervisor_slack',
        context: {
          leave,
          approver: supervisor,
          requester: leave.get('user'),
  ...
  const promise_slack_to_requestor = self
    .promise_rendered_slack_template({
      template_name: 'leave_request_edit_to_requestor_slack',
      context: {
        leave,
        approver: leave.get('approver'),
        requester: leave.get('user'),
