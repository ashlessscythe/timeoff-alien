...
function get_property_from_options(options, property_name) {
  ...
  if (
    ...
    Object.prototype.hasOwnProperty.call(options.data, 'root') &&
    Object.prototype.hasOwnProperty.call(options.data.root, property_name) &&
...
function get_current_format(options) {
  ...
  if (user && Object.prototype.hasOwnProperty.call(user, 'company')) {
...
function format_duration_from_times(time_start, time_end) {
  if (!time_start || !time_end) return ''

  const start = moment(time_start, ['HH:mm:ss', 'HH:mm'], true)
  const end = moment(time_end, ['HH:mm:ss', 'HH:mm'], true)
...
module.exports = function() {
  return {
    ...
    can_edit_pending_leave: function(leave, loggedUser, options) {
      if (!leave || !loggedUser) {
        return options.inverse(this)
      }

      const isPending =
        (typeof leave.is_new_leave === 'function' && leave.is_new_leave()) ||
        leave.status === 1
      ...
      if (Number(loggedUser.id) === Number(leave.user_id)) {
        return options.fn(this)
      }
    ...
    // Given start/end time (HH:mm:ss or HH:mm) return compact duration
    format_time_duration: function(time_start, time_end) {
      return format_duration_from_times(time_start, time_end)
    },
      ...
      try {
        const user =
          ...
          (options &&
            options.data &&
            options.data.root &&
            options.data.root.current_user) ||
          (options &&
            options.data &&
            options.data.root &&
            options.data.root.requester) ||
          (options &&
            options.data &&
            options.data.root &&
          ...
          if (user.shift_hours) {
            ...
            if (shifts.length > 0) {
              ...
              if (firstShift && firstShift.start && firstShift.end) {
                ...
                const end = moment(firstShift.end, 'HH:mm')
                if (end.isBefore(start)) {
                  end.add(1, 'day')
                }
    ...
    format_days_and_hours: function(fractional_days, format_type, options) {
      ...
      if (
        format_type &&
        typeof format_type === 'object' &&
        format_type.hash === undefined &&
        !format_type.fn
      ) {
        options = format_type
        format_type = 'verbose'
        ...
        !format_type ||
        (format_type !== 'compact' && format_type !== 'verbose')
      ) {
      ...
      if (absDays < 0.0001) {
        console.log(
          'format_days_and_hours: Value is effectively 0, returning 0d'
        )
      ...
      console.log(
        'format_days_and_hours: Processing value:',
        absDays,
        'whole_days will be:',
        Math.floor(absDays)
      )
      ...
      if (format_type === 'compact') {
        ...
        if (whole_days === 0 && absDays > 0) {
          ...
          const final_hours =
            calc_hours === 0 && precise_hours > 0 ? 1 : Math.max(1, calc_hours)
