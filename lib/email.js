...
// Initialize email transport based on available configuration
let emailTransport
if (process.env.RESEND_API_KEY) {
  ...
  if (smtpConfig.auth.required) {
    delete smtpConfig.auth.required
  } else {
    delete smtpConfig.auth
  }
  emailTransport = {
    type: 'smtp',
    client: nodemailer.createTransport(smtpTransport(smtpConfig))
  }
}
    ...
    try {
      if (emailTransport.type === 'resend') {
        const {
          data: emailData,
          error
        } = await emailTransport.client.emails.send({
          from: data.from,
          to: data.to,
