...
Email.prototype.promise_rendered_email_template = async function(args) {
  try {
    ...
    const renderedEmail = await handlebars.render(
      path.join(__dirname, '..', 'views', 'email', filename + '.hbs'),
      contextWithBranding
    )
    ...
    const final_email = await handlebars.render(
      path.join(__dirname, '..', 'views', 'email', 'wrapper.hbs'),
      {
    ...
    return function(data) {
      ...
      const subj =
        data && data.subject != null
          ? String(data.subject).slice(0, 80)
          : ''
      console.log('[send_email=false] would send email to', to, subj)
    ...
    try {
      ...
      try {
        if (emailTransport.type === 'resend') {
          ...
          const sendWithRetry = () => new Promise((resolve, reject) => {
      ...
      self
        ...
        .then(email_obj => {
          ...
          return send_mail({
            ...
            .then(() => bluebird.resolve({ new_user, token }))
            .catch(error => {
              console.error('Failed to send add new user email:', error)
              // Still return token so Slack can use the same one
              return bluebird.resolve({ new_user, token })
            })
        })
    )
    .catch(error => {
      console.error('Error in promise_add_new_user_email:', error)
      return bluebird.resolve({ new_user, token: null })
    })
}

Email.prototype.promise_leave_request_revoke_emails = async function(args) {
  const self = this
  ...
  let comments = []
  try {
    comments = await getCommentsForLeave({ leave })
  } catch (error) {
    console.error('Error loading comments for revoke emails:', error)
  }

  const sendToSupervisor = async () => {
    const approver = leave.get('approver')
    ...
    try {
      ...
      if (!email_obj || !email_obj.subject || !email_obj.body) {
        return
      }
      await approver.record_email_addressed_to_me(email_obj)
      await send_mail({
        from: config.get('email:smtp:from'),
        to: approver.email,
        subject: email_obj.subject,
  ...
  const sendToRequestor = async () => {
    const requester = leave.get('user')
    ...
    try {
      ...
      await requester.record_email_addressed_to_me(email_obj)
      await send_mail({
        from: config.get('email:from'),
        to: requester.email,
        subject: email_obj.subject,
  ...
  try {
    await Promise.all([sendToSupervisor(), sendToRequestor()])
  } catch (error) {
...
Email.prototype.promise_leave_request_emails = function(args) {
  ...
  const promise_email_to_supervisor = ({
    ...
    self
      ...
      .then(async email_obj => {
        // Always record the email in the database, even if sending fails
        await supervisor.record_email_addressed_to_me(email_obj)
...
Email.prototype.promise_leave_request_edit_emails = function(args) {
  const self = this
  const leave = args.leave
  ...
  const promise_email_to_supervisor = ({
    ...
    self
      .promise_rendered_email_template({
        template_name: 'leave_request_edit_to_supervisor',
        context: {
          leave,
          comments,
          approver: supervisor,
      ...
      .then(async email_obj => {
        ...
        return send_mail({
          ...
          console.error(
            'Failed to send leave edit email to supervisor:',
            error
          )
          return bluebird.resolve()
        })
      ...
      .catch(error => {
        console.error(
          'Error preparing leave edit email to supervisor:',
          error
        )
        return bluebird.resolve()
      })
  ...
  const promise_email_to_requestor = ({ comments, requesterAllowance }) =>
    self
      .promise_rendered_email_template({
        template_name: 'leave_request_edit_to_requestor',
        context: {
          leave,
          comments,
          approver: leave.get('approver'),
      ...
      .then(async email_obj => {
        ...
        return send_mail({
          ...
          console.error(
            'Failed to send leave edit email to requestor:',
            error
          )
          return bluebird.resolve()
        })
      ...
      .catch(error => {
        console.error(
          'Error preparing leave edit email to requestor:',
          error
        )
        return bluebird.resolve()
      })
  ...
  return Promise.all([
    ...
    .catch(error => {
      console.error('Error in promise_leave_request_edit_emails:', error)
      return bluebird.resolve()
    })
}
