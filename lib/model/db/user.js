  ...
  function get_class_methods(sequelize) {
    return {
      ...
      hashify_password: function(password) {
        ...
        const secret = config.get('crypto_secret')
        const encoding = config.get('crypto_hash_encoding') || 'binary'
        const result = crypto
          .createHash('md5')
          .update(password + secret, encoding)
          .digest('hex')

        console.log('User model hashing with:', {
          password,
          secret,
          encoding,
          result
        ...
        return (
          sequelize.models.User.find_by_email(attributes.email)
            .then(existing_user => {
              ...
              console.log('attribute.name is', attributes.name)
