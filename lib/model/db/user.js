  ...
  function get_instance_methods(sequelize) {
    return {
      ...
      get_reset_password_token: function() {
        const self = this
        return Buffer.from(self.email + ' ' + this.password).toString('base64')
      },
  ...
  function get_class_methods(sequelize) {
    return {
      ...
      get_user_by_reset_password_token: function(token) {
        ...
        const [email, stored_hash] = unpacked_token.split(/\s/)

        return self.find_by_email(email).then(user => {
          if (user && user.password === stored_hash) {
