  ...
  const Company = sequelize.define(
    'Company',
    {
      ...
      next_year_cutoff_date: {
        ...
        allowNull: true,
        comment:
          'Cutoff date after which limited departments can request PTO for next calendar year'
      },
      limited_departments: {
        type: DataTypes.TEXT,
        allowNull: true,
        comment:
          'JSON array of department IDs that are restricted from requesting next year PTO before cutoff date',
        set: function(val) {
          if (!val || typeof val !== 'object') {
            val = JSON.stringify([])
          } else {
