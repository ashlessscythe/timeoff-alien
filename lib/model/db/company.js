...
module.exports = function(sequelize, DataTypes) {
  const Company = sequelize.define(
    'Company',
    {
      ...
      company_wide_message_text_color: {
        ...
        allowNull: true,
        defaultValue: '#000000',
        comment: 'Text color for company-wide message',
        validate: {
          isHexColor: true
        }
      },
      company_wide_message_bg_color: {
        ...
        allowNull: true,
        defaultValue: '#f2dede',
        comment: 'Background color for company-wide message alert bar',
        validate: {
          isHexColor: true
        }
      },
      ...
      payroll_close_time: {
        ...
        allowNull: false,
        defaultValue: 10,
        comment: 'Time payroll closes on monday UTC'
      },
      ...
      last_name_first: {
        ...
        allowNull: false,
        defaultValue: false,
        comment: 'If true, last name will be displayed first'
      },
      first_day_of_week: {
        type: DataTypes.INTEGER,
        allowNull: false,
        defaultValue: 1,
        comment: 'First day of week: 0 for Sunday, 1 for Monday'
      },
      next_year_cutoff_date: {
        type: DataTypes.DATEONLY,
        allowNull: true,
        comment: 'Cutoff date after which limited departments can request PTO for next calendar year'
      },
      limited_departments: {
        type: DataTypes.TEXT,
        allowNull: true,
        comment: 'JSON array of department IDs that are restricted from requesting next year PTO before cutoff date',
        set: function(val) {
          if (!val || typeof val !== 'object') {
            val = JSON.stringify([])
          } else {
          ...
          this.setDataValue('limited_departments', val)
        },
        get: function() {
          let val = this.getDataValue('limited_departments')
          if (!val) {
            return []
          }
          try {
            ...
            console.error(
              'Failed to parse the limited_departments saved in company %s. Error: %s',
              this.id,
              err
            )
            val = []
          ...
          Promise.all(
            ...
            Promise.all(
              _.map(departments_to_update, department => {
                department.manager_id = id_maps.user[department.manager_id]
  ...
  Company.prototype.normalise_date = function(date_str) {
    console.log('Normalising date ' + date_str + ' for company ' + this.name)
    return moment
  ...
  Company.prototype.get_today = function() {
    // console.log('Getting today for company ' + this.name)
    const self = this
  ...
  Company.prototype.getDateFormat = function() {
    return this.date_format || 'YYYY-MM-DD' // Default format if not set
  }
