...
module.exports = function(sequelize, DataTypes) {
  const Department = sequelize.define(
    'Department',
    {
      ...
      allowed_increments: {
        type: DataTypes.TEXT,
        allowNull: true,
        defaultValue: '["full_day","half_day"]',
        comment: 'JSON array: ["full_day", "half_day", "hourly", "quarter_hr"]'
      },
      minimum_notice_by_leave_type: {
        type: DataTypes.TEXT,
        allowNull: true,
        comment: 'JSON object: {"leave_type_id": days} e.g. {"5": 3}'
      }
    },
    {
      underscored: true,
  ...
  Department.loadScope = function(models) {
    ...
    Department.addScope('with_manager', {
  ...
  // added ref after def
  Department.prototype.Model = Department
    ...
    return Promise.join(
      ...
      (company, users_and_leaves) => {
        ...
        users_and_leaves.forEach(user_data => {
          ...
          // filter bank holidays
          const filteredBankHolidays = company.bank_holidays.map(day => ({
            date: day.date,
            name: day.name
          }))

          // console.log('Filtered Bank Holidays:', filteredBankHolidays)
  ...
  Department.prototype.promise_me_with_supervisors = function() {
    ...
    if (self.Model) {
      ...
      // handle where undefined
      return Promise.reject(new Error('Department.Model is not defined'))
    }
  }
