...
module.exports = function(sequelize, DataTypes) {
  const Leave = sequelize.define(
    'Leave',
    {
      ...
      day_part_start: {
        type: DataTypes.INTEGER,
        allowNull: false,
        defaultValue: 1 // Legacy: 1=all day, 2=morning, 3=afternoon
      },
      time_start: {
        type: DataTypes.STRING(8),
        allowNull: true,
        comment: 'Start time for time-based leaves (e.g., "09:00:00")'
      },
      date_end: {
        type: DataTypes.DATE,
        allowNull: false,
      ...
      time_end: {
        type: DataTypes.STRING(8),
        allowNull: true,
        comment: 'End time for time-based leaves (e.g., "11:00:00")'
      }
    },
    {
      underscored: true,
    ...
    this._days = days.map(
      day =>
        new LeaveDay({
          ...
          // Pass time fields for time-based leaves
          time_start: day === start_date ? this.time_start : null,
          time_end: day === end_date ? this.time_end : null
        })
    )
  ...
  // Helper function to get working hours per day from user/department shift configuration
  // Returns default 8 hours if no shift configuration found
  Leave.prototype.get_working_hours_per_day = function(user) {
    try {
      // Check user-specific shift hours first
      if (user.shift_hours) {
        const userShiftHours =
          typeof user.shift_hours === 'string'
            ? JSON.parse(user.shift_hours)
            : user.shift_hours

        // Get the first shift's hours (or calculate average if multiple shifts)
        ...
        if (shifts.length > 0) {
          ...
          if (firstShift && firstShift.start && firstShift.end) {
            ...
            // Handle next-day shifts (e.g., swing shift ending at 01:00)
      ...
      // Fall back to department shift hours
      if (user.department && user.department.shift_hours) {
        const deptShiftHours =
  ...
  Leave.prototype.get_deducted_days_number = function(args) {
    args = args || {}
    const leave_days = this.get_deducted_days(args)
    let number_of_days = leave_days.length

    // Check if this is a time-based leave (has time_start or time_end)
    const is_time_based = !!(this.time_start || this.time_end)

    if (is_time_based) {
      // Calculate fractional days from time-based leave
      const user = this.user || this.approver || args.user
      const working_hours_per_day = this.get_working_hours_per_day(user)

      let total_hours = 0

      leave_days.forEach(leave_day => {
        if (leave_day.is_time_based_leave()) {
          const hours = leave_day.get_duration_hours()
          if (hours !== null) {
            total_hours += hours
          } else {
            // If time-based but can't calculate hours, treat as full day
            total_hours += working_hours_per_day
          }
        } else {
          // Legacy day_part logic for non-time-based days
          if (leave_day.is_all_day_leave()) {
            total_hours += working_hours_per_day
          } else {
            // Half day
            total_hours += working_hours_per_day / 2
          }
        }
      })

      // Convert total hours to fractional days
      return total_hours / working_hours_per_day
    } else {
      // Legacy day_part logic
              ...
              return user
                ...
                .then(overlapping_leaves => {
                  if (overlapping_leaves.length > 0) {
                    const toUtcDate = d => moment.utc(d).format('YYYY-MM-DD')

                    const makeDateTime = (dateStr, timeStr) =>
                      moment.utc(`${dateStr}T${timeStr}`)

                    const dayIntervalForPart = (dateStr, dayPart) => {
                      const startOfDay = moment.utc(dateStr).startOf('day')
                      const endOfDay = moment.utc(dateStr).add(1, 'day').startOf('day')

                      const all = String(sequelize.models.Leave.leave_day_part_all())
                      const morning = String(sequelize.models.Leave.leave_day_part_morning())
                      const afternoon = String(
                        sequelize.models.Leave.leave_day_part_afternoon()
                      )

                      if (String(dayPart) === all) {
                      ...
                      if (String(dayPart) === morning) {
                        return { start: startOfDay, end: midday }
                      }
                      if (String(dayPart) === afternoon) {
                        return { start: midday, end: endOfDay }
                      }

                      return { start: startOfDay, end: endOfDay }
                    }

                    const buildIntervalsForLeaveLike = leaveLike => {
                      const fromDate = toUtcDate(
                        leaveLike.from_date || leaveLike.date_start
                      )
                      const toDate = toUtcDate(leaveLike.to_date || leaveLike.date_end)
                      ...
                      for (let i = 0; i <= daysSpan; i++) {
                        ...
                        if (daysSpan === 0 && (timeStart || timeEnd)) {
                          if (timeStart && timeEnd) {
                            const s = makeDateTime(day, timeStart)
                            let e = makeDateTime(day, timeEnd)
                            if (!e.isAfter(s)) {
                              e = e.add(1, 'day')
                            }
                            ...
                            intervals.push(
                              dayIntervalForPart(
                                day,
                                sequelize.models.Leave.leave_day_part_all()
                              )
                            )
                          }
                          continue
                        ...
                        const dayPart =
                          i === 0
                            ...
                            : i === daysSpan
                              ? toPart
                              : sequelize.models.Leave.leave_day_part_all()
                        intervals.push(dayIntervalForPart(day, dayPart))
                      }
                    ...
                    const anyOverlap = (aIntervals, bIntervals) => {
                    ...
                    const selfIntervals = buildIntervalsForLeaveLike(self)
                    const conflicting = overlapping_leaves.find(overlapping_leave => {
                      const otherIntervals = buildIntervalsForLeaveLike(overlapping_leave)
                      return anyOverlap(selfIntervals, otherIntervals)
                    })
