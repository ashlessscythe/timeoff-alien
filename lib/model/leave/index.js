...
const { Leave, User, LeaveType } = require('../../prisma/models')
...
async function createNewLeave(args) {
  ...
  const new_leave_status = (leave_type.auto_approve || employee.auto_approve)
    ? 2 // approved
    : 1 // new

  // Create leave object (not saved to database yet)
  const leave_to_create = {
    user_id: employee.id,
  ...
  const leave = await Leave.create(leave_to_create)
