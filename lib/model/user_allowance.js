...
const schema_promise_allowance = Joi.object({
  ...
  // Whether pending (status_new) requests should count as already taken.
  // Use `true` when validating a new request; use `false` when validating approval.
...
const scheme_constructor = Joi.object({
  ...
  number_of_days_taken_personal: Joi.number().required(),
  number_of_days_taken_regular: Joi.number().required(),
...
class UserAllowance {
  constructor(args) {
    ...
    self._number_of_days_taken_personal = args.number_of_days_taken_personal
    self._number_of_days_taken_regular = args.number_of_days_taken_regular
  ...
  toJSON() {
    return {
      // totals
      total_number_of_days_in_allowance: this.total_number_of_days_in_allowance,
      number_of_days_available_in_allowance: this.number_of_days_available_in_allowance,
      number_of_days_taken_from_allowance: this.number_of_days_taken_from_allowance,

      // personal/vacation split
      nominal_personal: this.nominal_personal,
      total_personal_taken: this.total_personal_taken,
      total_personal_available: this.total_personal_available,
      vacation_remaining: this.vacation_remaining,

      // regular pool details
      number_of_regular_allowance: this.number_of_regular_allowance,
      remaining_regular_allowance: this.remaining_regular_allowance,

      // adjustments / metadata
      nominal_allowance: this.nominal_allowance,
      manual_adjustment: this.manual_adjustment,
      personal_adjustment: this.personal_adjustment,
      carry_over: this.carry_over,

      // accrued semantics
      is_accrued_allowance: this.is_accrued_allowance,
      employement_range_adjustment: this.employement_range_adjustment,
      accrued_adjustment: this.accrued_adjustment
    }
  }
  ...
  get total_personal_available() {
    ...
    return Math.max(
      0,
      this._nominal_personal +
        this._personal_adjustment -
        this._number_of_days_taken_personal
    )
  }

  get total_personal_taken() {
    return this._number_of_days_taken_personal
  }

  get vacation_remaining() {
    const self = this
    return (
      self.number_of_days_available_in_allowance - self.total_personal_available
    )
  }
  ...
  get total_number_of_days_in_allowance() {
    ...
    return (
      ...
      (self.is_accrued_allowance ? self.employement_range_adjustment : 0)
    )
  }
  ...
  get remaining_regular_allowance() {
    ...
    return Math.max(
      0,
      this.number_of_regular_allowance - this._number_of_days_taken_regular
    )
  }
  ...
  get is_accrued_allowance() {
    return !!(this.user.department && this.user.department.is_accrued_allowance)
  }
  ...
  get employement_range_adjustment() {
    ...
    // If department does not use accrued allowance, employment date based
    // adjustments should not apply or show up in breakdown.
    if (!self.is_accrued_allowance) {
      return 0
    }
  ...
  static promise_allowance(args) {
    ...
    const include_pending = args.include_pending

    let flow = Promise.resolve()

    // Ensure `my_leaves` are loaded for the requested year.
    // Many code paths initialize `my_leaves` to `[]` (not `undefined`), which
    // previously prevented reloading and caused taken-days to be calculated as 0.
    const requestedYear =
      year && typeof year.year === 'function' ? year.year() : moment.utc(year).year()
    if (
      !Array.isArray(user.my_leaves) ||
      user._my_leaves_year !== requestedYear
    ) {
    ...
    flow = flow.then(() => {
      const number_of_days_taken_personal = user.calculate_number_of_days_taken_from_allowance(
        {
          year: year.format('YYYY'),
          count_personal: true,
          exclude_leave_id,
      ...
      const number_of_days_taken_regular = user.calculate_number_of_days_taken_from_allowance(
        {
          year: year.format('YYYY'),
          count_regular: true,
          exclude_leave_id,
      ...
      const args = {
        ...
        number_of_days_taken_personal,
        number_of_days_taken_regular,
