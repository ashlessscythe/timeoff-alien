...
class UserAllowance {
  ...
  get vacation_remaining() {
    // Vacation-only balance: do not derive from (total available − personal), or
    // canceling a personal leave changes personal remaining and incorrectly moves
    // this number. remaining_regular_allowance is (regular pool − vacation-only taken).
    return this.remaining_regular_allowance
  }
  ...
  get number_of_days_available_in_allowance() {
    ...
    return Math.max(0, calculatedAllowance)
  }
  ...
  get accrued_adjustment() {
    ...
    return Number((-1 * (Math.round(delta * 2) / 2)).toFixed(1))
  }
  ...
  static promise_allowance(args) {
    ...
    // Always refresh leaves for this year. Parallel callers (e.g. calendar route)
    // may have stale or empty my_leaves; allowance must include pending (new) leaves.
