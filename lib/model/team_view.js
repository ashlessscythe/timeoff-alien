...
TeamView.prototype.promise_team_view_details = function(args) {
  ...
  const department_ids = args.department_ids || [] // array of department IDs
  ...
  return promise_departments.then(departments => 
    // IMPORTANT:
    // Team view fans out into many DB queries (per department and per user).
    // With small Prisma pool sizes, running all departments fully in parallel
    // quickly exhausts the pool and triggers timeouts. Limit concurrency here.
     Promise.map(
      departments,
      department =>
