...
const fetchLeavesForLeavesReport = async ({
  ...
  // By default, leaves report excludes rejected and canceled leaves. Allow callers
  // to override this by passing an explicit statusFilter.
  if (statusFilter && Array.isArray(statusFilter) && statusFilter.length > 0) {
    whereClause.status = {
      [dbModel.Sequelize.Op.in]: statusFilter
    }
  } else {
    whereClause.status = {
...
const fetchLeavesForHeatmap = async ({
  ...
  const { User, Leave, LeaveType, Department, Company } = dbModel
  ...
  // Build a map of dates to user counts and details
  const leavesByDate = {}
  const leavesDetails = {}
  ...
  leaves.forEach(leave => {
    ...
    while (dateKey.isSameOrBefore(end, 'day')) {
      ...
      // Check if this user is already counted for this day
      const existingUser = leavesByDate[dateStr].users.find(
        u => u.id === leave.user.id
      )

      if (!existingUser) {
        const userName =
          leave.user && leave.user.full_name
            ? leave.user.full_name()
            : leave.user
            ? `${leave.user.name || ''} ${leave.user.lastname || ''}`.trim()
            : 'N/A'
      ...
      // Store detailed leave information
