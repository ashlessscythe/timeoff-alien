  ...
  static promiseToRemove(args) {
    ...
    return (
      User.findOne({
        ...
        // .. delete email audit
        .then(() => EmailAudit.destroy({ where: { company_id: company.id } }))
        ...
        .then(() =>
          company
            ...
            .then(users =>
              Promise.all(
                users.map(u => Leave.destroy({ where: { user_id: u.id } }))
              )
            )
        )
        // Remove all users
        .then(() => User.destroy({ where: { company_id: company.id } }))
