...
module.exports = function(sequelize) {
  ...
  this.validate_overlapping = function(new_leave_attributes) {
    ...
    const makeDateTime = (dateStr, timeStr) => {
      ...
      return moment.utc(`${dateStr}T${timeStr}`)
    }
    ...
    const dayIntervalForPart = (dateStr, dayPart) => {
      ...
      const afternoon = String(sequelize.models.Leave.leave_day_part_afternoon())

      if (String(dayPart) === all) {
        return { start: startOfDay, end: endOfDay }
      }
      // Legacy halves: treat as [00:00,12:00) and [12:00,24:00)
      ...
      // Fallback to full day if unexpected value
      return { start: startOfDay, end: endOfDay }
    }

    const buildIntervalsForAttributes = attrs => {
      const fromDate = toUtcDate(attrs.from_date || attrs.date_start)
      const toDate = toUtcDate(attrs.to_date || attrs.date_end)

      const fromPart = attrs.from_date_part || attrs.day_part_start
      const toPart = attrs.to_date_part || attrs.day_part_end

      const timeStart = attrs.time_start || null
      const timeEnd = attrs.time_end || null
      ...
      for (let i = 0; i <= daysSpan; i++) {
        const day = moment.utc(fromDate).add(i, 'days').format('YYYY-MM-DD')

        // If time-based is provided for single-day leaves, use time interval.
        // Otherwise fall back to legacy day_part intervals (including halves).
        if (daysSpan === 0 && (timeStart || timeEnd)) {
          if (timeStart && timeEnd) {
            const s = makeDateTime(day, timeStart)
            let e = makeDateTime(day, timeEnd)
            // Allow next-day end time (e.g., swing shift)
            if (!e.isAfter(s)) {
              e = e.add(1, 'day')
            }
            intervals.push({ start: s, end: e })
          } else {
            // If incomplete time range, treat as full day to be safe
            intervals.push(dayIntervalForPart(day, sequelize.models.Leave.leave_day_part_all()))
          }
          continue
        }

        // Multi-day: apply day parts on boundaries; middle days are full day
        const dayPart =
          i === 0
            ? fromPart
            : i === daysSpan
    ...
    const intervalsOverlap = (a, b) => a.start.isBefore(b.end) && b.start.isBefore(a.end)
    ...
    return (
      this_user
        ...
        .then(overlapping_leaves => {
          ...
          const newIntervals = buildIntervalsForAttributes(new_leave_attributes)

          // If any existing leave overlaps the new leave in time, reject.
          // Legacy half-day "fits" are naturally allowed because their intervals do not overlap.
          const conflicting = overlapping_leaves.find(existing_leave => {
            const existingIntervals = buildIntervalsForAttributes({
              date_start: existing_leave.date_start,
              date_end: existing_leave.date_end,
              day_part_start: existing_leave.day_part_start,
              day_part_end: existing_leave.day_part_end,
              time_start: existing_leave.time_start,
              time_end: existing_leave.time_end
            })
            return anyOverlap(newIntervals, existingIntervals)
          })
