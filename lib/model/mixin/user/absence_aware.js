...
module.exports = function(sequelize) {
  ...
  this.get_leave_statistics_by_types = async function(args) {
    ...
    // Get personal adjustment and total personal days taken first
    const personal_adjustment = await self.promise_personal_adjustment_for_year(year)
    const total_personal_days_taken = self.calculate_number_of_days_taken_from_allowance({
      year: year,
      count_personal: true
    })
    const total_personal_allowance = self.department.personal + (personal_adjustment || 0)
    ...
    for (const leave_type of company.leave_types) {
      ...
      if (leave_type.use_personal) {
        initial_stat.limit = total_personal_allowance
      } else {
