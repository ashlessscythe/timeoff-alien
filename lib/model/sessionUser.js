...
function wrapDepartment(row, prismaClient) {
  ...
  department._promise_team_view = async function(args) {
    ...
    const users_and_leaves = await Promise.all(
      _.map(users, async user => {
        const leaves = await user.promise_my_leaves_for_calendar({
          year: start_date
        })
...
function wrapUser(row, prismaClient) {
  ...
  user.promise_to_update_personal_adjustment = async function(args) {
    ...
    return this._prisma.user_allowance_adjustment.upsert({
      ...
      create: {
        user_id: this.id,
        year: asInt(args.year),
        personal_adjustment: args.adjustment,
        created_at: now()
      },
      update: {
        personal_adjustment: args.adjustment
  ...
  user.calculate_number_of_days_taken_from_allowance = function(args) {
    ...
    if (args.count_personal) {
      ...
      // Default (UserAllowance pool): vacation-allowance draws only. Types that use
      // BOTH use_allowance and use_personal are counted only under count_personal so
      // the same calendar day is not subtracted from the vacation pool and again from personal.
      leaves = leaves.filter(
        leave =>
          leave.leave_type &&
          leave.leave_type.use_allowance &&
  ...
  user.get_leave_statistics_by_types = async function(args) {
    ...
    _.filter(
      ...
      leave =>
        (leave.is_approved_leave() || leave.is_new_leave()) &&
        leave.status !== leaveConstants.status_canceled() &&
        leave.status !== leaveConstants.status_rejected()
    ).forEach(leave => {
  ...
  user.validate_leave_fits_into_remaining_allowance = async function(args) {
    ...
    if (allowance.number_of_days_available_in_allowance - deductedDays < 0) {
      const error = new Error('Requested absence is longer than remaining allowance')
