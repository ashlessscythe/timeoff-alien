...
const { sortLeaveTypes } = require('../util/leaveTypeSort')
...
function wrapCompany(row, prismaClient) {
  ...
  company.leave_types = sortLeaveTypes(
    (row.leave_types || []).map(lt => wrapLeaveType(lt, prismaClient)),
    company
  )
  ...
  company.getSortedLeaveTypes = function() {
    return sortLeaveTypes(this.leave_types || [], this)
  }
...
function wrapDepartment(row, prismaClient) {
  ...
  department.promise_team_view_for_month = function(month, options) {
    return this._promise_team_view(
      Object.assign({ start_date: month }, options || {})
    )
  }

  department.promise_team_view_for_months_range = function(
    start_month,
    end_month,
    options
  ) {
  ...
  department._promise_team_view = async function(args) {
    ...
    if (!start_date) {
      const companyForToday = args.company || (await this.getCompany())
    ...
    // Light user rows (department only) — company comes from args/session.
    const users = (
    ...
    ).map(user => wrapUser(user, prisma))

    // Only leaves that overlap the displayed month range (not the whole year).
    ...
    const [leaveRows, schedulesByUserId, company] = await Promise.all([
      userIds.length === 0
        ? Promise.resolve([])
      ...
      loadSchedulesForUsers(prisma, userIds, this.company_id),
      args.company ? Promise.resolve(args.company) : this.getCompany()
    ])
...
async function loadSchedulesForUsers(prismaClient, userIds, companyId) {
  ...
  const companyDefault =
    schedules.find(schedule => schedule.user_id === null) || null
  const userSchedules = schedules.filter(schedule => schedule.user_id)

  return userIds.reduce((acc, userId) => {
    const userSchedule = userSchedules.find(schedule => schedule.user_id === userId)
    acc[userId] = wrapSchedule(userSchedule || companyDefault || null, prismaClient)
    return acc
  }, {})
}
...
function wrapLeave(row, prismaClient) {
  ...
  // Prisma uses created_at; views/helpers often expect Sequelize-style createdAt.
  if (leave.createdAt == null && leave.created_at != null) {
    leave.createdAt = leave.created_at
  }
  if (leave.editedAt == null && leave.edited_at != null) {
    leave.editedAt = leave.edited_at
  }
  ...
  leave.save = async function(opts) {
    const data = {
      status: this.status,
    ...
    if (this.edited_at !== undefined) {
      data.edited_at = this.edited_at
    }

    // Skip heavy joins when caller already attached associates (bookleave path).
    const hasAssociates = !!(this.leave_type && this.user && this.approver)
    const includeAssociates =
      opts && opts.includeAssociates !== undefined
        ? !!opts.includeAssociates
        : !hasAssociates

    const queryOpts = includeAssociates
      ? { include: getLeaveIncludeForPrisma() }
      : undefined
    ...
    if (!includeAssociates) {
      if (preserved.leave_type) this.leave_type = preserved.leave_type
      if (preserved.user) this.user = preserved.user
      if (preserved.approver) this.approver = preserved.approver
    }
    return this
  }
  ...
  return (
    await prismaClient.leaves.findMany({
      ...
      include:
        args.include === false
          ? undefined
          : args.include || getLeaveIncludeForPrisma(),
...
function wrapUser(row, prismaClient) {
  ...
  user.is_my_password_async = function(password) {
    return User.verify_password_async(password, this.password)
  }

  user.maybe_activate = async function() {
    if (this.activated) {
      return this
    }
    this.activated = true
  ...
  user.is_admin = function() {
    return this.admin === true || this.admin === 1
  }
  ...
  user.save = async function() {
    ...
    if (data.email_change_expires !== undefined && data.email_change_expires !== null) {
      if (typeof data.email_change_expires === 'string') {
        data.email_change_expires = coerceUserDateTimeField(data.email_change_expires)
      }
    }
  ...
  user.reload_minimal_session_details = async function() {
    if (!this.company || !this.company.get_today) {
      const companyRow =
        this.companies ||
        (await this._prisma.companies.findUnique({
          where: { id: this.company_id }
        }))
      this.company = wrapCompany(companyRow, this._prisma)
    }
    ...
    if (!Array.isArray(this.supervised_users)) {
      this.supervised_users = []
    }

    return this
  }

  user.ensure_supervised_users = async function() {
    if (this._supervised_users_loaded) return this

    if (!this.is_admin() && !this.is_manager()) {
      this.supervised_users = []
      this._supervised_users_loaded = true
      return this
    }
  ...
  user.get_company_with_all_leave_types = function() {
    if (this._company_with_leave_types_loaded && this.company) {
      return Promise.resolve(this.company)
    }

    return this.getCompany().then(company => {
      this.company = company
      this._company_with_leave_types_loaded = true
      return company
    })
  }

  // Lighter company load for leave booking: leave types + bank holidays only.
  // Avoids pulling every department row on submit.
  user.ensure_company_with_leave_types = async function() {
    if (this._company_with_leave_types_loaded && this.company) return this

    // Only trust already-attached leave types when they are the fully wrapped
    // records (with model methods like get_color_value). Middleware such as the
    // booking-modal loader attaches lightweight plain rows (id/name/sort_order
    // only) to company.leave_types, which must NOT be treated as fully loaded —
    // otherwise consumers like CalendarMonth crash on get_color_value().
    if (
      this.company &&
      Array.isArray(this.company.leave_types) &&
      this.company.leave_types.length > 0 &&
      typeof this.company.leave_types[0].get_color_value === 'function'
    ) {
      this._company_with_leave_types_loaded = true
      return this
    }

    const row = await this._prisma.companies.findUnique({
      where: { id: this.company_id },
      include: {
        leave_types: {
          orderBy: [{ sort_order: 'desc' }, { name: 'asc' }]
    ...
    this.company = wrapCompany(row, this._prisma)
    this._company_with_leave_types_loaded = true
    return this
  }

  user.reload_with_session_details = async function() {
    await this.ensure_supervised_users()
    await this.ensure_company_with_leave_types()
    this.cached_schedule = await this.promise_schedule_I_obey()
  ...
  user.reload_with_leave_details = async function(args) {
    ...
    const leaves = await this.promise_my_active_leaves({
      year: args.year,
      light
    })

    this.my_leaves = light
      ? leaves
      : await LeaveCollectionUtil.enrichLeavesWithComments({
          leaves,
          dbModel: {}
        })
    ...
    // Reuse department/schedule when already hydrated in promise_my_leaves.
    this.department = await this.getDepartment()
    this.cached_schedule = await this.promise_schedule_I_obey()
  ...
  user.promise_users_I_can_manage = async function() {
    ...
    if (this.is_admin()) {
      // Lightweight rows for booking modal / lists — avoid hydrating every
      // user's company+department graphs (was dominating login→calendar).
      users = (
        await this._prisma.users.findMany({
          where: { company_id: this.company_id },
          select: {
  ...
  user.promise_manager = async function() {
    ...
    if (Number(department.manager_id) === Number(this.id)) {
      return this
    }
  ...
  user.promise_my_leaves = async function(args) {
    ...
    // Allowance checks only need leave_type + the owning user's schedule /
    // department / bank holidays (already on `this`). Avoid the heavy
    // per-leave user/approver/company joins used by calendar views.
    const include = args.light
      ? { leave_types: true }
    ...
    // Hydrate once — callers historically re-queried department/schedule/approver
    // per leave, which dominated leave-submit allowance checks on Neon.
    const department = this.department || (await this.getDepartment())
  ...
  user.validate_overlapping = async function(newLeaveAttributes, options) {
    options = options || {}
    const excludeLeaveId = options.exclude_leave_id
    ...
    const leaves = await findLeaves(this._prisma, {
      ...
      include: false // date/time fields only — enough for interval overlap checks
    })

    const candidates = excludeLeaveId
      ? leaves.filter(
          existingLeave => String(existingLeave.id) !== String(excludeLeaveId)
        )
      : leaves
    ...
    const conflicting = candidates.find(existingLeave =>
  ...
  user.validate_leave_fits_into_remaining_allowance = async function(args) {
    ...
    const employee = await this.reload_with_leave_details({ year, light: true })
    // Allowance math needs department, schedule, and bank holidays — already
    // covered by reload_with_leave_details + bank holidays below. Avoid
    // reload_with_session_details() which loads every supervised user.
    if (
      !employee.company ||
      !employee.company.reload_with_bank_holidays ||
      !Array.isArray(employee.company.bank_holidays)
    ) {
      if (!employee.company || !employee.company.reload_with_bank_holidays) {
        employee.company = await employee.getCompany()
      }
  ...
  user.record_email_addressed_to_me = function(emailObj, opts) {
    ...
    const recipientEmail =
      ...
      String(opts.recipientEmail).trim() !== ''
        ? String(opts.recipientEmail).trim()
        : this.email
...
const User = {
  ...
  async find_for_auth(email, prismaClient) {
    const db = getPrisma(prismaClient)
    ...
    const user = wrapUser(row, db)
    user.company = wrapCompany(row.companies, db)
    return user
  },
