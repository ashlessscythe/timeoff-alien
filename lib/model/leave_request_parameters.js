...
function LeaveRequest(args) {
  ...
  // Note: time_start and time_end are optional (for time-based leaves)
  _.each(
    [
      'leave_type',
      'from_date',
    ...
    property => {
      if (!_.has(args, property)) {
        throw new Error(
          'No mandatory ' +
            property +
            ' was provided to LeaveRequest constructor'
        )
      }
    }
  )
  ...
  if (moment.utc(args.from_date).toDate() > moment.utc(args.to_date).toDate()) {
    throw new Error(
      'From date should be before To date at LeaveRequest constructor'
    )
  }
...
LeaveRequest.prototype.is_within_one_day = function() {
  return (
    moment.utc(this.from_date).format('YYYY-MM-DD') ===
...
LeaveRequest.prototype._does_fit_with_point = function(leave_day, point_name) {
  ...
  if (
    ...
    String(this[point_name + '_part']) !==
      String(model.Leave.leave_day_part_all()) &&
