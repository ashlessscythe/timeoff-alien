...
router.get('/leave-edit/:leaveId/', async (req, res) => {
  try {
    const leaveId = validator.trim(req.params.leaveId)
    const leave = await getLeaveForUserView({ actingUser: req.user, leaveId })

    if (!canUserEditPendingLeave({ actingUser: req.user, leave })) {
      return res.status(403).json({ error: 'You are not allowed to edit this leave request' })
    }
    ...
    return res.json({
      ...
      ...leaveToFormFields(leave)
    })
  } catch (error) {
    console.error('Failed to load leave for edit:', error)
    return res.status(404).json({ error: 'Leave request not found' })
  }
})

router.post('/editleave/', async (req, res) => {
  try {
    const requestId = validator.trim(req.body.request)
    if (!requestId || !validator.isNumeric(String(requestId))) {
      req.session.flash_error('Invalid leave request')
    ...
    const existingLeave = await getLeaveForUserView({
      actingUser: req.user,
      leaveId: requestId
    })

    if (!canUserEditPendingLeave({ actingUser: req.user, leave: existingLeave })) {
      Exception.throwUserError({
        user_error: 'You are not allowed to edit this leave request',
        system_error:
          'User ' + req.user.id + ' attempted to edit leave ' + requestId
      })
    }
    ...
    if (
      leave_type.manager_only &&
      !(req.user.is_admin() || req.user.is_manager())
    ) {
      Exception.throwUserError({
        user_error: 'This leave type is restricted to managers',
        system_error:
          'User ' +
          req.user.id +
          ' attempted to edit to manager_only leave type ' +
          leave_type.id
      })
    }
    ...
    if (company.is_mode_readonly_holidays()) {
      req.session.flash_error(
        'Company account is locked and leave requests could not be updated. Please contact administration.'
      )
      throw new Error('Company is in "Read-only holidays" mode')
    }

    const leave = await updatePendingLeave({
    ...
    if (reloadedLeave._edit_had_substantive_changes) {
      const Slack = new SlackTransport()
      Slack.promise_leave_request_edit_slacks({
        leave: reloadedLeave,
        edited_by: req.user
      }).catch(error => {
        console.error('Failed to send leave edit slack:', error)
      })

      const Email = new EmailTransport()
      Email.promise_leave_request_edit_emails({
        leave: reloadedLeave,
        edited_by: req.user
      }).catch(error => {
        console.error('Failed to send leave edit email:', error)
      })
    }

    req.session.flash_message('Leave request was updated')
    return res.redirect_with_session(
      req.body.redirect_back_to ? req.body.redirect_back_to : '../'
    )
  } catch (error) {
    ...
    if (error.user_message) {
      ...
      req.session.flash_error(error.user_error)
    } else {
      req.session.flash_error('Failed to update leave request')
    }
...
router.get('/teamview/', async (req, res) => {
  ...
  // Leave types + bank holidays are enough for TeamView calendars/statistics.
  ...
  try {
    const team_view_details = await team_view.promise_team_view_details({
      department_ids,
      company
    })
...
router.get('/leave-summary/:leaveId/', async (req, res) => {
  ...
  try {
    ...
    if (extendedView) {
      ...
      leaveDetails.canEdit = canUserEditPendingLeave({
        actingUser,
        leave
      })
