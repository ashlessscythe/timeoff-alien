...
router.post('/bookleave/', (req, res) => {
  Promise.join(
    ...
    (users, company, valid_attributes) => {
      ...
      const from_date = moment(valid_attributes.from_date)
      const to_date = moment(valid_attributes.to_date)
      const requested_days = to_date.diff(from_date, 'days') + 1

      // Load employee details and validate leave balance
      return Promise.join(
        employee.reload_with_leave_details({ year: from_date }),
        employee.getCompany({ include: ['leave_types'] }),
        (employee_with_leaves, employee_company) => {
          // Ensure employee has company and leave types loaded
          employee.company = employee_company

          // Get leave statistics
          const leave_statistics = employee.get_leave_statistics_by_types()
          const type_stat = leave_statistics.find(
            stat => stat.leave_type.id === leave_type.id
          )

          if (type_stat) {
            const remaining = type_stat.limit - type_stat.days_taken
            if (requested_days > remaining) {
              const error = new Error(
                'Requested days exceed remaining allowance'
              )
              error.user_message = `Not enough ${
                leave_type.name
              } days remaining. You have ${remaining} days left.`
              throw error
            }
          }
