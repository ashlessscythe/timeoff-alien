...
const parseAllowedIncrements = department => {
  ...
  return DEFAULT_ALLOWED_INCREMENTS
}

const getAllowedIncrementsForEmployee = async employee => {
  if (employee && employee.department_id) {
    try {
      const department = await prisma.departments.findUnique({
        where: { id: employee.department_id },
        select: { allowed_increments: true }
      })
      if (department) {
        return parseAllowedIncrements({
          id: employee.department_id,
          allowed_increments: department.allowed_increments
        })
      }
    } catch (error) {
      console.error(
        'Failed to load department allowed_increments via Prisma for employee ' +
          employee.id +
          ':',
        error
      )
  ...
  try {
    const employeeDepartment = await employee.getDepartment()
    return parseAllowedIncrements(employeeDepartment)
  } catch (error) {
    console.error(
      'Failed to load department allowed_increments via Sequelize for employee ' +
        employee.id +
        ':',
      error
    )
...
const getEffectiveMinimumNotice = async (employee, leaveTypeId, leaveTypeDefault) => {
  let deptNotice = null
  try {
    const dept = await prisma.departments.findUnique({
      where: { id: employee.department_id },
      select: { minimum_notice_by_leave_type: true }
    })
    if (dept && dept.minimum_notice_by_leave_type) {
      const parsed = JSON.parse(dept.minimum_notice_by_leave_type)
      if (parsed && typeof parsed === 'object') {
        deptNotice =
          parsed[leaveTypeId] ??
          parsed[String(leaveTypeId)]
      }
    }
  } catch (_err) {
    try {
      const d = await employee.getDepartment()
      if (d && d.minimum_notice_by_leave_type) {
        const parsed = JSON.parse(d.minimum_notice_by_leave_type)
        if (parsed && typeof parsed === 'object') {
          deptNotice =
            parsed[leaveTypeId] ?? parsed[String(leaveTypeId)]
        }
      }
    } catch (_e) {
      // ignore
  ...
  const effective =
    deptNotice !== undefined && deptNotice !== null && deptNotice !== ''
      ? parseInt(deptNotice, 10)
      : (leaveTypeDefault ?? 0)
  return isNaN(effective) ? 0 : Math.max(0, effective)
}

const getRequestedIncrement = ({
  incrementType,
  incrementValue,
  fromDatePart,
  timeStart,
...
router.post('/bookleave/', (req, res) => {
  Promise.join(
    ...
    async (users, company, valid_attributes) => {
      ...
      const allowedIncrements = await getAllowedIncrementsForEmployee(employee)

      if (!allowedIncrements.includes(requestedIncrement)) {
        Exception.throwUserError({
          user_error:
          ...
          system_error:
            ...
            (employeeDepartment ? employeeDepartment.id : 'unknown')
        })
      }
      ...
      const minimumNotice = await getEffectiveMinimumNotice(
        employee,
        leave_type.id,
        leave_type.minimum_days_notice
      )
      if (minimumNotice > 0) {
        const fromDateStr = valid_attributes.from_date
        const today = req.user.company.get_today()
        const fromMoment = moment.utc(fromDateStr, 'YYYY-MM-DD').startOf('day')
        const todayMoment = moment.utc(today).startOf('day')
        const daysUntil = fromMoment.diff(todayMoment, 'days')
        if (daysUntil < minimumNotice) {
          Exception.throwUserError({
            user_error:
              'This request is short notice. This leave type requires ' +
              minimumNotice +
              ' day(s) prior notice. Please submit at least ' +
              minimumNotice +
              ' days before the first day of leave.',
            system_error:
              'Leave request for user ' +
              employee.id +
              ' failed minimum notice: required ' +
              minimumNotice +
              ', got ' +
              daysUntil +
              ' days'
...
router.get('/', (req, res) => {
  ...
  Promise.join(
    ...
    async (calendar, company, user, supervisors, user_allowance) => {
      // Ensure user has all necessary data loaded
      await user.reload_with_session_details()
      await user.get_company_with_all_leave_types()
  ...
  try {
    ...
    if (extendedView) {
      ...
      leaveDetails.canApprove =
        ...
        (leave.status === leave.constructor.status_new() ||
          leave.status === leave.constructor.status_pended_revoke())
