...
function resolveBookLeaveEmployee(req, valid_attributes) {
  const hasUserIndex =
  ...
  const index = parseInt(valid_attributes.user, 10)
  if (Number.isNaN(index) || index < 0 || index >= users.length) {
    return users[valid_attributes.user] || req.user
  }

  return users[index] || req.user
}
...
router.post('/bookleave/', async (req, res) => {
  try {
    ...
    const employee = resolveBookLeaveEmployee(req, valid_attributes)
    ...
    const reloadedLeave = await leave.reloadWithAssociates()
