...
const getExplicitLeaveTypeIdsForEmployee = async employee => {
  if (!employee || !employee.department_id) {
    return []
  }
  ...
  try {
    ...
    return parseExplicitLeaveTypeIds(department)
  } catch (error) {
    console.error(
      'Failed to load department leave types for employee ' +
...
router.post('/bookleave/', (req, res) => {
  Promise.join(
    ...
    async (users, company, valid_attributes) => {
      ...
      const explicitLeaveTypeIds = await getExplicitLeaveTypeIdsForEmployee(
        employee
      )
      if (
        !isLeaveTypeAllowedForDepartment({
