...
const renderRejectionsReportAsCsv = async ({
  res,
  company,
  startDate,
  endDate,
  ...
  // Compose file name
  res.attachment(
    `${company.name_for_machine()}_rejections_report_between_${startDate.format(
      'YYYY_MM_DD'
    )}_and_${endDate.format('YYYY_MM_DD')}.csv`
  )
...
router.get('/rejections/', async (req, res) => {
  ...
  try {
    ...
    // Reuse fetchLeavesForLeavesReport but override the status filter to only include
    // rejected and canceled leaves, so we can leverage existing mapping logic.
    const { Leave } = dbModel
    ...
    const { leaves } = await fetchLeavesForLeavesReport({
      ...
      leaveTypeId,
      companyDateFormat: company.get_default_date_format(),
      // Custom status filter handled inside Report.js when provided
      statusFilter: [Leave.status_rejected(), Leave.status_canceled()]
    })

    const sortedLeaves = leaves.sort(getSorterForLeaves(sortBy))

    if (renderAsCsv) {
      await renderRejectionsReportAsCsv({
        res,
        company,
        startDate,
        endDate,
    ...
    console.error(
      `An error occurred when user ${
        actingUser.id
      } tried to access /reports/rejections/ page: ${error} at ${error.stack}`
    )

    const userErrorMessage = error.tom_error
      ? Exception.extract_user_error_message(error)
      : 'Failed to produce Rejected & Canceled Leaves report. Please contact administrator.'
...
router.get('/heatmap/', async (req, res) => {
  ...
  let startMonth = moment.utc()

  if (monthParam) {
    // Try to parse as YYYY-MM format
    const parsedMonth = moment.utc(monthParam, 'YYYY-MM', true)
  ...
  const startDate = startMonth.clone().startOf('month')
  const endDate = startMonth
    .clone()
    .add(2, 'months')
    .endOf('month')
  ...
  try {
    ...
    const { leavesByDate, leavesDetails } = await fetchLeavesForHeatmap({
      actingUser,
      dbModel,
      startDate,
      endDate,
    ...
    // Get company's first day of week (0 = Sunday, 1 = Monday)
    const firstDayOfWeek =
      company.first_day_of_week !== undefined ? company.first_day_of_week : 1

    // Prepare calendar months data (3 months)
    const calendarMonths = []
    for (let i = 0; i < 3; i++) {
      const monthDate = startMonth.clone().add(i, 'months')
      const monthStart = monthDate.clone().startOf('month')
      const monthEnd = monthDate.clone().endOf('month')

      // Calculate first day of calendar grid based on company setting
      let firstDay = monthStart.clone()
      const monthStartDayOfWeek = firstDay.day() // 0 = Sunday, 1 = Monday, etc.

      // Calculate offset to get to the first day of the calendar week
      let offset = 0
      if (firstDayOfWeek === 0) {
        // Sunday start: go back to the Sunday before or on the first day of month
        offset = -monthStartDayOfWeek
      } else {
        // Monday start: go back to the Monday before or on the first day of month
        // If month starts on Sunday (0), go back 6 days; otherwise go back (monthStartDayOfWeek - 1) days
        offset = monthStartDayOfWeek === 0 ? -6 : 1 - monthStartDayOfWeek
      }
      firstDay = firstDay.add(offset, 'days')

      // Calculate last day of calendar grid (6 weeks later)
      const lastDay = firstDay.clone().add(41, 'days') // 6 weeks * 7 days - 1
      ...
      while (currentDay.isSameOrBefore(lastDay, 'day')) {
        ...
        const isOtherMonth = currentDay.month() !== monthDate.month()
        const isWeekend = currentDay.day() === 0 || currentDay.day() === 6
        const dayData = leavesByDate[dateStr] || { count: 0, users: [] }

        // Calculate heatmap color based on count
        let heatmapColor = '#fff'
        if (dayData.count > 0) {
          if (dayData.count === 1) heatmapColor = '#c6e48b'
          else if (dayData.count === 2) heatmapColor = '#7bc96f'
          else if (dayData.count === 3) heatmapColor = '#239a3b'
          else if (dayData.count === 4) heatmapColor = '#196127'
          else if (dayData.count >= 5 && dayData.count <= 6)
            heatmapColor = '#ffa500'
          else if (dayData.count >= 7 && dayData.count <= 9)
            heatmapColor = '#ff6b00'
          else heatmapColor = '#cc0000'
        }
    ...
    // Calculate navigation months (move by 3 months to show next/previous 3-month period)
    const prevMonth = startMonth
      .clone()
      .subtract(3, 'months')
      .format('YYYY-MM')
    ...
    // Prepare weekday labels based on first day of week
    const weekdayLabels =
      firstDayOfWeek === 0
        ? ['S', 'M', 'T', 'W', 'T', 'F', 'S']
        : ['M', 'T', 'W', 'T', 'F', 'S', 'S']

    res.render('report/heatmap', {
      leavesByDate: JSON.stringify(leavesByDate),
      leavesDetails: JSON.stringify(leavesDetails),
      calendarMonths,
      startDateObj: startDate,
      endDateObj: endDate,
      currentMonth: startMonth.format('YYYY-MM'),
      prevMonth,
      nextMonth,
      currentYear,
      firstDayOfWeek,
      weekdayLabels,
      company: actingUser.company,
      title: 'Absences Heat Map | TimeOff'
    })
  } catch (error) {
    console.error(
      `An error occurred when user ${
        actingUser.id
      } tried to access /reports/heatmap/ page: ${error} at ${error.stack}`
    )

    const userErrorMessage = error.tom_error
      ? Exception.extract_user_error_message(error)
      : 'Failed to produce Heat Map report. Please contact administrator.'
