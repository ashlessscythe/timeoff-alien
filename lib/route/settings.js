...
router.get('/leavetypes/', async (req, res) => {
  try {
    ...
    console.error('An error occurred while fetching leave types settings:', error)
...
function findBestMatch(csvName, csvLastname, users) {
  ...
  for (const user of users) {
    ...
    const score = Math.max(fullNameScore, reversedScore, (nameScore + lastnameScore) / 2)
...
router.post('/pto-adjustment/preview/', uploadMemory.single('pto_import'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const header = parsed_data.shift().map(h => (h || '').toLowerCase().trim())
    ...
    if (nameIndex === -1 || lastnameIndex === -1 || adjustmentIndex === -1 || yearIndex === -1) {
      req.session.flash_error(
    ...
    for (let i = 0; i < parsed_data.length; i++) {
      ...
      const personalAdjustmentStr =
        personalAdjustmentIndex !== -1 ? (row[personalAdjustmentIndex] || '').trim() : '0'
...
router.post('/pto-adjustment/apply/', async (req, res) => {
  try {
    ...
    console.log('Type:', typeof confirmedRowsData, 'IsArray:', Array.isArray(confirmedRowsData))
    ...
    if (confirmedRows.length === 0) {
      req.session.flash_error('No adjustments selected. Please select at least one adjustment to apply.')
      return res.redirect_with_session('/settings/pto-adjustment/')
    }
    ...
    for (const item of itemsToApply) {
      try {
        ...
        console.error(`Error updating adjustment for user ${item.matchedUserId}:`, error)
      }
    }
