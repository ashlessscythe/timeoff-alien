...
router.get('/leavetypes/', async (req, res) => {
  try {
    ...
    // Fetch the company with leave types
    const company = await req.user.getCompany({
    ...
    .then(company => {
      ...
      return Promise.all([
        ...
        _.map(company.leave_types, (leave_type, index) => {
          ...
          // Update leave type only if there are attributes submitted for it (Sequelize primary, Prisma sync)
          if (!attributes) return Promise.resolve(1)
          return leave_type.update(attributes).then(updated => {
            return prisma.leave_types
              .update({
                where: { id: leave_type.id },
                data: {
                  name: attributes.name,
                  color: attributes.color,
                  use_allowance: attributes.use_allowance,
                  use_personal: attributes.use_personal,
                  auto_approve: attributes.auto_approve,
                  manager_only: attributes.manager_only,
                  is_special: attributes.is_special,
                  limit: attributes.limit,
                  sort_order: attributes.sort_order,
                  allow_non_default_increments:
                    attributes.allow_non_default_increments,
                  minimum_days_notice: attributes.minimum_days_notice ?? 0
                }
              })
              .then(() => updated)
              .catch(err => {
                console.error(
                  'Prisma leave_types sync failed for id ' + leave_type.id,
                  err
                )
                return updated
              })
...
function get_and_validate_leave_type(args) {
  ...
  const allow_non_default_increments =
    ...
    false
  let minimum_days_notice = 0
  const rawMinNotice = req.body['minimum_days_notice__' + suffix]
  if (
    rawMinNotice !== undefined &&
    rawMinNotice !== null &&
    String(rawMinNotice).trim() !== ''
  ) {
    const parsed = parseInt(rawMinNotice, 10)
    minimum_days_notice = isNaN(parsed) ? 0 : parsed
  }
  ...
  if (
    typeof minimum_days_notice !== 'number' ||
    isNaN(minimum_days_notice) ||
    minimum_days_notice < 0
  ) {
    throw_user_error(
      'Minimum days notice for ' + item_name + ' should be 0 or a positive number'
    )
  }
  ...
  return {
    ...
    is_special,
    allow_non_default_increments,
    minimum_days_notice: Math.max(0, minimum_days_notice)
  }
}
