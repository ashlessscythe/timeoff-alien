...
module.exports = function(passport) {
  ...
  router.post('/login', async (req, res, next) => {
    ...
    passport.authenticate('local', (err, user) => {
      ...
      req.logIn(user, err => {
        ...
        // Skip the `/` hop (which only redirects to calendar) — saves a full
        // session deserialize + booking-modal middleware round-trip.
  ...
  router.get('/forgot-password/sent/', (req, res) => {
    res.render('forgot_password_sent', {
      url_to_the_site_root: get_url_to_site_root_for_anonymous_session(req),
      title: 'Password reset email sent | TimeOff'
    })
  })
  ...
  router.post('/forgot-password/', async (req, res) => {
    ...
    req.app
      ...
      .then(user => {
        const Slack = new SlackTransport()
        const Email = new EmailTransport()
        const reset_requested_at = moment_tz
          .utc()
          .format('D MMM YYYY HH:mm') + ' UTC'

        // Generate the reset token once, then share it with Slack and email
        return user.get_reset_password_token().then(token => {
          Slack.promise_forgot_password_slack({
            ...
            console.error(
              'Failed to send forgot password Slack message: ' + error,
              error.stack
            )
          })
      ...
      .then(() => res.redirect_with_session('/forgot-password/sent/'))
      .catch(error => {
        if (error.do_not_report) {
          return res.redirect_with_session('/forgot-password/sent/')
        }
    ...
    req.app
      ...
      .then(user => {
        if (!user) {
          ...
          res.redirect_with_session('/forgot-password/')
          return null
        }
