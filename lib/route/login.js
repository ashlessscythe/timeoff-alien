  ...
  router.post('/forgot-password/', async (req, res) => {
    ...
    req.app
      ...
      .then(user => {
        ...
        return user.get_reset_password_token().then(token => user
            .getCompany()
            .then(company => Email.promise_rendered_email_template({
            ...
            .then(email_obj => Email.get_send_email()({
                from: config.get('email:from'),
                to: user.email,
                subject: email_obj.subject,
                html: email_obj.body
              }).then(send_results => user
