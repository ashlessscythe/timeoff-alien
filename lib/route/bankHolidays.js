  ...
  req.user
    ...
    .then(company => {
      ...
      const bankHolidays = company.bank_holidays
        ...
        .map(bh => ({
          id: bh.id, // Include the id property
          // updated to include name
          date: moment.utc(bh.date).format('YYYY-MM-DD'),
          name: bh.name
        }))

      // log map
      console.log('mapped bh: ', bankHolidays)

      const calendar = [...Array(12).keys()]
        .map(i => i + 1)
        .map(
          m =>
            new CalendarMonth(`${currentYear}-${String(m).padStart(2, '0')}`, {
              today,
              schedule: {
...
router.post('/bankholidays/', (req, res) => {
  ...
  req.user
    ...
    .then(async company => {
      const dateFormat = company.getDateFormat();

      const normalizeDate = (date) => {
        const formattedDate = moment(date, dateFormat, true);
        if (!formattedDate.isValid()) {
          throw new Error(`Invalid date format, expected format is ${dateFormat}`);
        }
        return formattedDate.format('YYYY-MM-DD'); // normalize to YYYY-MM-DD for storage
      };
      ...
      const allBankHolidays = company.bank_holidays
        .filter(bankHoliday => req.body[`date__${bankHoliday.id}`] !== undefined)
        .map(bankHoliday => {
          bankHoliday.name = req.body[`name__${bankHoliday.id}`] || bankHoliday.name;
          bankHoliday.date = normalizeDate(req.body[`date__${bankHoliday.id}`]) || bankHoliday.date;
          return bankHoliday;
        });
      ...
      if (req.body.name__new && validator.trim(req.body.name__new)) {
        allBankHolidays.push({
          ...
          date: normalizeDate(req.body.date__new),
      ...
      const invalidBankHolidays = allBankHolidays.filter(bankHoliday => {
        const date = bankHoliday.date;
        return !moment(date, 'YYYY-MM-DD', true).isValid(); // ensure the normalized date is valid
      });

      // Handle errors
      if (invalidBankHolidays.length > 0) {
        invalidBankHolidays.forEach(bankHoliday =>
          req.session.flash_error(`New day for ${bankHoliday.name} should be a valid date`)
        );
        // If there were any validation errors: do not update bank holidays
