...
router.get('/bankholidays/', (req, res) => {
  ...
  req.user
    ...
    .then(company => {
      ...
      console.log(
        'BankHolidays route: company.first_day_of_week =',
    ...
    .then(async company => {
      ...
      const allBankHolidays = company.bank_holidays
        ...
        .map(bankHoliday => {
          ...
          bankHoliday.date =
            normalizeDate(req.body[`date__${bankHoliday.id}`]) ||
            bankHoliday.date
          return bankHoliday
        })
      ...
      if (invalidBankHolidays.length > 0) {
        invalidBankHolidays.forEach(bankHoliday =>
          req.session.flash_error(
            `New day for ${bankHoliday.name} should be a valid date`
          )
        )
