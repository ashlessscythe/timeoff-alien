...
// Managers can access the list route (/), but admin-only routes are protected below
router.all(/.*/, (req, res, next) => {
  ...
  // Allow admin or manager for the list route (/)
  ...
  // For all other routes, require admin
...
router.get('/', (req, res) => {
  ...
  req.user
    .getCompany({
      ...
      // Disable subQuery to allow better use of database indexes
      subQuery: false,
      order: [
    ...
    .then(args => {
      // Include breakdown when CSV is requested
      const includeBreakdown = !!req.query['as-csv']
      return promise_user_list_data_for_rendering(args, includeBreakdown)
    })
...
/**
 * Efficiently calculate personal vs vacation remaining breakdown for all users using Prisma
 * This is optimized for batch processing to avoid N+1 queries
 */
async function calculateAllowanceBreakdownForUsers(users_info, year) {
  if (!users_info || users_info.length === 0) {
    return {}
  }

  const userIds = users_info.map(ui => ui.user_row.id)
  ...
  // Fetch all leaves for all users in one query
  ...
  // Fetch all adjustments for all users in one query
  ...
  const departmentIds = [
    ...new Set(users_info.map(ui => ui.user_row.department.id))
  ]
  ...
  adjustments.forEach(adj => {
    adjustmentMap.set(adj.user_id, adj)
  })
  ...
  leaves.forEach(leave => {
    if (!leavesByUser.has(leave.user_id)) {
      leavesByUser.set(leave.user_id, [])
    }
    leavesByUser.get(leave.user_id).push(leave)
  })

  // Calculate breakdown for each user
  const breakdown = {}

  users_info.forEach(ui => {
    const userId = ui.user_row.id
    const adjustment = adjustmentMap.get(userId) || {}
    const department = departmentMap.get(ui.user_row.department.id)
    ...
    const totalPersonalLimit = department.personal + personalAdjustment

    // Calculate personal days taken using the same logic as calculate_number_of_days_taken_from_allowance
    // This is a simplified calculation - for exact accuracy, we'd need to use the full get_deducted_days_number logic
    // which considers working days, schedules, bank holidays, and half days
    const userLeaves = leavesByUser.get(userId) || []
    let personalDaysTaken = 0

    userLeaves.forEach(leave => {
      // Only count leaves that use personal allowance
      if (leave.leave_types.use_personal) {
        // Simplified calculation: count calendar days
        // Note: This is an approximation. The actual calculation would exclude weekends,
        // bank holidays, and handle half days based on user's schedule
        const days =
          moment(leave.date_end).diff(moment(leave.date_start), 'days') + 1
        personalDaysTaken += days
      }
    })
    ...
    const personalRemaining = Math.max(
      0,
      totalPersonalLimit - personalDaysTaken
    )

    // Vacation remaining = total remaining - personal remaining
    // Personal days are a subset of total allowance, so vacation remaining is the difference
    const remainingAllowance = ui.number_of_days_available_in_allowance

    // Ensure vacation remaining doesn't go negative
    // If personal remaining exceeds total remaining, vacation is 0
    const vacationRemaining = Math.max(
      0,
      remainingAllowance - personalRemaining
    )
...
function promise_user_list_data_for_rendering(args, includeBreakdown = false) {
  const company = args[0]
  const users_info = args[1]
  const year = company.get_today()

  const baseData = users_info.map(ui => ({
  ...
  if (includeBreakdown) {
    // Calculate breakdown using Prisma for efficient batch processing
    return calculateAllowanceBreakdownForUsers(users_info, year).then(
      breakdown => {
        const usersInfoForRendering = baseData.map(ui => ({
          ...ui,
          personal_remaining: breakdown[ui.user_id]?.personal_remaining || 0,
          vacation_remaining: breakdown[ui.user_id]?.vacation_remaining || 0
        }))
    ...
    const sortedUsersInfoForRendering = baseData.sort((a, b) =>
      sorter(a.user_lastname, b.user_lastname)
    )
...
function users_list_as_csv(args) {
  ...
  users_info.forEach(ui => {
    content.push([
      ...
      ui.department_name,
      ui.number_of_days_available_in_allowance,
      ui.personal_remaining !== undefined ? ui.personal_remaining : '',
      ui.vacation_remaining !== undefined ? ui.vacation_remaining : '',
...
function get_and_validate_user_parameters(args) {
  ...
  const is_allowed_adjustment_increment = value => {
    const abs_value = Math.abs(Number(value))
    const increment = 0.125
    const remainder = abs_value % increment
    const epsilon = 1e-8

    return remainder < epsilon || Math.abs(remainder - increment) < epsilon
  }
  ...
  } else if (adjustment && !is_allowed_adjustment_increment(adjustment)) {
    req.session.flash_error(
      'New allowance adjustment of ' +
        item_name +
        ' should be whole, half-day, or hourly (0.125)'
    )
  }
  ...
  if (personal_adjustment && !validator.isFloat(personal_adjustment)) {
    ...
    personal_adjustment &&
    !is_allowed_adjustment_increment(personal_adjustment)
  ) {
    req.session.flash_error(
      'New personal days adjustment of ' +
        item_name +
