    ...
    .then(new_user =>
      Email.promise_add_new_user_email({
        ...
        .catch(error => {
          ...
          return Promise.resolve({ new_user, token: null })
        })
    )
...
router.get('/edit/:user_id/absences/', (req, res) => {
  ...
  Promise.try(() => ensure_user_id_is_integer({ req, user_id }))
    ...
    .then(employee =>
      Promise.join(
        employee
          .promise_allowance({ year, include_pending: true })
          .then(allowance_obj =>
            Promise.resolve([
              (user_allowance =
                allowance_obj && typeof allowance_obj.toJSON === 'function'
                  ? allowance_obj.toJSON()
                  : allowance_obj),
              employee
            ])
          ),
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  try {
    ...
    const uaRaw = await employee.promise_allowance({ year, include_pending: true })
    userAllowance = uaRaw && typeof uaRaw.toJSON === 'function' ? uaRaw.toJSON() : uaRaw
  } catch (error) {
    console.error(
      `An error ocurred while trying to render Calendar for user [${user_id}]: ${error} at ${error.stack
      }`
    )
...
router.post('/edit/:user_id/email/send-signin/', async (req, res) => {
  ...
  try {
    ...
    const reset_requested_at =
      moment.utc().format('D MMM YYYY HH:mm') + ' UTC'

    const email_obj = await Email.promise_rendered_email_template({
      template_name: 'forgot_password',
      context: { user: targetUser, company, token, reset_requested_at }
    })
...
require('./users_list_prisma')(router)
...
function get_and_validate_user_parameters(args) {
  ...
  const adjustment =
    validator.isFloat(String(params.adjustment || '').trim())
      ? parseFloat(params.adjustment)
      : null
  const personal_adjustment =
    validator.isFloat(String(params.personal_adjustment || '').trim())
      ? parseFloat(params.personal_adjustment)
      : null
  ...
  const is_valid_number = value =>
    typeof value === 'number' && Number.isFinite(value)

  if (adjustment !== null && adjustment !== undefined && !is_valid_number(adjustment)) {
    req.session.flash_error(
    ...
    !is_allowed_adjustment_increment(adjustment)
  ) {
    req.session.flash_error(
  ...
  if (
    personal_adjustment !== null &&
    personal_adjustment !== undefined &&
    !is_valid_number(personal_adjustment)
  ) {
    req.session.flash_error(
