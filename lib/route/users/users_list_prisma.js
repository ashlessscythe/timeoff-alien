    ...
    try {
      ...
      const users_info = await Promise.all(
        users.map(async user => {
          ...
          // Keep calculation aligned with the canonical allowance pipeline:
          // allowance_obj includes pending; "used/taken" must exclude pending.
          ...
          return {
            ...
            allowance: {
              ...allowance_obj,
              // For CSV/export: keep "remaining" (available) deducting pending,
              // but make "days_used" exclude pending.
