  ...
  return function(req, res) {
    ...
    Promise.try(() => req.user.promise_leaves_to_be_processed())
      ...
      .then(processed_leave => {
        // save comment to db (kinda hacky, but oh well)
        processed_leave.approver_comment = comment // adding directly
        return processed_leave.save() // multiple saves
      })
      .then(processed_leave =>
...
router.post('/cancel/', (req, res) => {
  ...
  Promise.try(() => req.user.promise_cancelable_leaves())
    ...
    .then(leave => {
      // Only attempt to send email if user has an email
    ...
    .then(leave => {
      // Only attempt to send slack notification if user has a slack username
    ...
    .catch(error => {
      ...
      // Even if there was an error with notifications, if we have a canceled leave, it was successful
      if (canceledLeave) {
        req.session.flash_message('The leave request was canceled')
      } else {
...
router.post('/revoke/', (req, res) => {
  ...
  Promise
    ...
    .then(leave_to_process => {
      ...
      // Check if leave is in past pay period (weekly)
      const leaveStartDate = moment(leave_to_process.date_start)
      const currentDate = moment()

      // If leave start date is in a past week and user is not admin
      if (
    ...
    .catch(error => {
      ...
      // Use custom user message if available, otherwise use generic message
