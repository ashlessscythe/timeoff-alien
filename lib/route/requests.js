...
router.get('/', (req, res) => {
  ...
  const viewAll = req.query.viewAll === 'true' && req.user.is_admin()

  // Function to get leaves based on admin status and view preference
  const getLeavesToProcess = () => {
    if (viewAll && req.user.is_admin()) {
      return dbModel.Leave.findAll({
        include: [
          {
            model: dbModel.User,
            as: 'user',
            include: [
              ...
              },
              {
                ...
                as: 'company',
                include: [{ model: dbModel.BankHoliday, as: 'bank_holidays' }]
              }
            ]
          },
          { model: dbModel.LeaveType, as: 'leave_type' },
          { model: dbModel.User, as: 'approver' }
        ],
        where: {
          status: [
            dbModel.Leave.status_new(),
            dbModel.Leave.status_pended_revoke()
          ]
        },
  ...
  Promise.join(
    ...
    getLeavesToProcess()
      .then(leaves =>
        Promise.all(
          leaves.map(leave =>
            leave.user.promise_schedule_I_obey().then(() => leave)
          )
        )
      )
      .then(leaves =>
