...
} = require('../model/payrollCloseWindow')
...
router.post('/revoke/', (req, res) => {
  ...
  if (!request_id || !validator.isNumeric(request_id) || Number.isNaN(leaveId)) {
  ...
  Promise.try(() =>
    req.app.get('db_model').Leave.findOne({ where: { id: leaveId } })
  )
    .then(requested_leave => {
      if (!requested_leave) {
        ...
        err.user_message = 'Failed to revoke leave request'
        throw err
      }

      if (
        String(requested_leave.user.company_id) !==
    ...
    .then(leave_to_process => {
      ...
      const company = leave_to_process.user.company
      const payrollHour =
        company && company.payroll_close_time != null
          ? company.payroll_close_time
          : 10

      if (
        isPastPayrollWeekClosedForLeaveStart({
          leaveStart: leave_to_process.date_start,
          payrollCloseHourUtc: payrollHour
        }) &&
        !req.user.is_admin()
      ) {
