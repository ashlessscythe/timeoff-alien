...
router.get('/', (req, res) => {
  ...
  Promise.join(
    ...
    (my_leaves_grouped, to_be_approved_leaves, company) => {
      ...
      // Filter leaves based on selected departments and types
      const filtered_leaves =
        selectedDepartments.length || selectedTypes.length
          ? to_be_approved_leaves.filter(leave => {
              const departmentMatch =
                selectedDepartments.length === 0 ||
                (leave.user &&
                  leave.user.department &&
                  selectedDepartments.includes(
                    String(leave.user.department.id)
                  ))
              const typeMatch =
