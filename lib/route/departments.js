...
function get_and_validate_department(args) {
  ...
  // Build minimum_notice_by_leave_type from form (minimum_notice_days__<leave_type_id>)
  const minimum_notice_by_leave_type = {}
  if (req.body && typeof req.body === 'object') {
    Object.keys(req.body).forEach(key => {
      if (key.startsWith('minimum_notice_days__')) {
        const leaveTypeId = key.replace('minimum_notice_days__', '')
        const val = req.body[key]
        if (
          val !== undefined &&
          val !== null &&
          val !== '' &&
          validator.isInt(String(val)) &&
          parseInt(val, 10) >= 0
        ) {
          minimum_notice_by_leave_type[leaveTypeId] = parseInt(val, 10)
        }
      }
    })
  }
  ...
  return {
    ...
    minimum_notice_by_leave_type:
      Object.keys(minimum_notice_by_leave_type).length > 0
        ? JSON.stringify(minimum_notice_by_leave_type)
        : null
  }
}
...
async function promise_to_extract_company_and_department(
  ...
  // Parse minimum_notice_by_leave_type JSON for template
  let minimum_notice_by_leave_type_map = {}
  if (department.minimum_notice_by_leave_type) {
    try {
      const parsed = JSON.parse(department.minimum_notice_by_leave_type)
      if (parsed && typeof parsed === 'object') {
        minimum_notice_by_leave_type_map = parsed
      }
    } catch (_e) {
      // ignore
    }
  ...
  const departmentData = {
    ...
    allowed_increments_array: department.allowed_increments
      ...
      : ['full_day', 'half_day'],
    minimum_notice_by_leave_type_map
  }
...
router.get('/departments/edit/:department_id/', async (req, res) => {
  ...
  try {
    ...
    const leave_types_raw = await prisma.leave_types.findMany({
      where: { company_id: company.id },
      orderBy: { name: 'asc' },
      select: { id: true, name: true }
    })
    const leave_types = leave_types_raw.map(lt => ({
      id: lt.id,
      name: lt.name,
      minimum_days:
        department.minimum_notice_by_leave_type_map[lt.id] ??
        department.minimum_notice_by_leave_type_map[String(lt.id)] ??
        0
    }))
