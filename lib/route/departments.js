...
const { parseExplicitLeaveTypeIds } = require('./leaveTypeRules')
...
function parse_allowed_leave_type_ids_from_body(req) {
  let ids = []
  if (req.body['allowed_leave_type_ids[]']) {
    ids = Array.isArray(req.body['allowed_leave_type_ids[]'])
      ? req.body['allowed_leave_type_ids[]']
      : [req.body['allowed_leave_type_ids[]']]
  } else if (req.body.allowed_leave_type_ids) {
    ids = Array.isArray(req.body.allowed_leave_type_ids)
      ? req.body.allowed_leave_type_ids
      : [req.body.allowed_leave_type_ids]
  }
  ...
  return ids
    .map(id => parseInt(validator.trim(String(id)), 10))
    .filter(id => !isNaN(id))
}

function validate_allowed_leave_type_ids(args) {
  const req = args.req
  const companyLeaveTypeIds = args.companyLeaveTypeIds || []
  const departmentName = args.department_name || 'department'
  const selectedIds = parse_allowed_leave_type_ids_from_body(req)

  if (selectedIds.length === 0) {
    req.session.flash_error(
      'Select at least one leave type for ' + departmentName
    )
    return null
  }

  const validIdSet = new Set(companyLeaveTypeIds.map(id => Number(id)))
  const invalid = selectedIds.filter(id => !validIdSet.has(id))
  if (invalid.length > 0) {
    req.session.flash_error(
      'One or more leave types for ' +
        departmentName +
        ' are invalid or do not belong to this company'
    )
    return null
  }
...
router.get('/departments/', async (req, res) => {
  try {
    ...
    // Add users to company object for template compatibility with full_name
    company.users = users.map(user => ({
      ...user,
...
async function promise_to_extract_company_and_department(
  req,
  only_active = true
) {
  const department_id =
  ...
  company.leave_types = sortLeaveTypes(leaveTypes, company)
  ...
  const departmentData = {
    ...department,
    manager,
    supervisors: department.department_supervisors.map(ds => ({
      ...ds.users,
      full_name: userUtils.fullName(ds.users)
    })),
    ...
    allowed_increments_array: department.allowed_increments
      ...
      : ['full_day', 'half_day'],
    allowed_leave_type_ids: explicitLeaveTypeIds,
    restricts_leave_types: explicitLeaveTypeIds.length > 0
  }

  // Add users to company for template compatibility with full_name
  company.users = users.map(user => ({
    ...user,
    full_name: userUtils.fullName(user)
  }))
...
async function promise_to_update_supervisors(args) {
  ...
  // Use native Promise.all instead of Bluebird's Promise.all for Prisma promises
  await global.Promise.all([...removePromises, ...addPromises])
}
...
async function promise_to_update_department(args) {
  ...
  const companyLeaveTypes = await prisma.leave_types.findMany({
    where: { company_id: company.id },
    select: { id: true }
  })
  const companyLeaveTypeIds = companyLeaveTypes.map(lt => lt.id)

  const selectedLeaveTypeIds = validate_allowed_leave_type_ids({
    req,
    companyLeaveTypeIds,
    department_name: department.name
  })
  ...
  if (req.session.flash_has_errors()) {
    throw new Error(
      'Invalid leave type selection while attempting to update department details'
    )
  }

  const allSelected =
    selectedLeaveTypeIds.length === companyLeaveTypeIds.length &&
    companyLeaveTypeIds.every(id => selectedLeaveTypeIds.includes(id))
  ...
  await prisma.$transaction(async tx => {
    ...
    await tx.department_leave_types.deleteMany({
      where: { department_id: department.id }
    })

    if (!allSelected) {
      await tx.department_leave_types.createMany({
        data: selectedLeaveTypeIds.map(leave_type_id => ({
          department_id: department.id,
          leave_type_id
        }))
      })
...
router.get(
  '/departments/available-supervisors/:department_id/',
  async (req, res) => {
    const department_id = req.params.department_id
    ...
    try {
      ...
      const availableSupervisors = _.map(
        ...
        user => {
          return {
            ...user,
            _marked: supervisor_map[user.id],
            full_name: userUtils.fullName(user)
          }
        }
      )
