...
const { loadSessionUserById } = require('../model/sessionUser')
...
function prepare_user_for_session(args) {
  ...
  user
    .maybe_activate()
    .then(activatedUser => {
      // Session store only persists user id; deserializeUser re-hydrates on the
      // next request. Skip schedule reload here when auth already attached
      // company + department (common local-login path).
      if (
        activatedUser.company &&
        typeof activatedUser.company.get_today === 'function' &&
        activatedUser.department
      ) {
        if (!Array.isArray(activatedUser.supervised_users)) {
          activatedUser.supervised_users = []
        }
        return activatedUser
      }
      return activatedUser.reload_minimal_session_details()
    })
    .then(() => {
      done(null, user)
    })
...
function authenticate_user({ user, password, done }) {
  ...
  // Check if the provided password is correct (async bcrypt — don't block
  // the event loop for ~200ms on each login).
  Promise.resolve(user.is_my_password_async(password))
    .then(ok => {
      if (!ok) {
        console.error(
...
function strategy_handler(email, password, strategy, done) {
  ...
  model.User.find_for_auth(email)
    .then(user => {
...
module.exports = function() {
  ...
  passport.use(
    new BearerStrategy((token, done) => {
      getCompanyAdminByToken({ token, model })
        .then(user => user.reload_minimal_session_details())
  ...
  passport.deserializeUser((id, done) => {
    loadSessionUserById(prisma, id)
      .then(user => {
        if (!user || !user.id) {
          return null
        }
        return user.reload_minimal_session_details()
      })
      .then(user => {
